...
class CommentRequestValidator extends AbstractRequestValidator
{
    public function validate(array $requestData): array
    {
        ...
        if ($postId <= 0) {
            $this->addFieldError('id_ever_post', $this->transAdmin('Ce champ est obligatoire.'));
        } elseif (!$this->existsInModuleTable('ever_blog_post', 'id_ever_post', $postId)) {
            $this->addFieldError('id_ever_post', $this->transAdmin('Article introuvable (id : %id%).', ['%id%' => $postId]));
        }

        $nickname = trim((string) ($requestData['nickname'] ?? $requestData['name'] ?? ''));
        if ('' === $nickname) {
            $this->addFieldError('nickname', $this->transAdmin('Le nom est obligatoire.'));
        }

        $content = trim((string) ($requestData['content'] ?? $requestData['comment'] ?? ''));
        if ('' === $content) {
            $this->addFieldError('content', $this->transAdmin('Le commentaire est obligatoire.'));
        }
